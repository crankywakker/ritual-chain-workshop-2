...
contract RitualPredict {
    ...
    function createMarket(
        ...
        // ── Convert durations to block counts ─────────────────────────────
    ...
    function onScheduledResolve(
        ...
        // Unknown market: silently skip so the execution does not revert
        // (which would roll back the attempt counter).
        if (marketId == 0 || marketId > marketCount) return;
        Market storage m = _markets[marketId];
        ...
        // Pick a fresh TEE executor for each attempt.
        ...
        Outcome outcome = yesWins ? Outcome.Yes : Outcome.No;
        uint256 winningPool = yesWins ? m.totalYes : m.totalNo;

        // ── Pari-mutuel: if no one backed the winning side, refund ────────
        if (winningPool == 0) {
            _invalidate(m, marketId, "no stakes on winning side");
            return;
        }
        ...
        // Cancel the remaining scheduled executions to save fees.
    ...
    function _readOracle(
        ...
        // ── Encode the HTTP precompile call ───────────────────────────────
        // Layout: (method, url, headers, body, executor, ttlBlocks)
        bytes memory httpPayload = abi.encode(
            RitualChain.HTTP_GET,
            m.oracleUrl,
            new string[](0),   // headers
            "",                 // body
            executor,
            HTTP_TTL_BLOCKS
        );

        // ── Fire HTTP precompile (short-running async) ─────────────────
        (bool httpOk, bytes memory raw) = RitualChain.HTTP_PRECOMPILE.call(
            httpPayload
        );
        if (!httpOk) {
        ...
        // ── Decode the async envelope (reverts on malformed input) ────────
        // Use external `try` so a malformed response is a caught failure,
        // not a revert that would roll back the attempt counter.
        uint16 status;
        bytes memory body;
        ...
        try this.decodeHttpResponse(raw) returns (
            ...
            return (false, 0, "could not decode http response");
        }
        ...
        if (status != 200) {
            return (false, 0, "non-200 http status");
        }
        if (body.length == 0) {
            return (false, 0, "empty http body");
        }

        // ── Run jq synchronously ──────────────────────────────────────────
        string memory bodyStr = string(body);
        (bool jqOk, uint256 parsed) = _jqUint(m.jsonPath, bodyStr);
        if (!jqOk) {
            return (false, 0, "jq parse failed");
        }
    ...
    function _pickExecutor(
        ...
        // Seed from block hash + market id + attempt index so each retry
        // has a different seed and is likely to land on a different executor.
        ...
        // If the registry has no live executor, return address(0). The HTTP
        // precompile will fail and _fail() will handle the retry/invalidation.
    ...
    function _scheduleResolution(
        ...
        // The Scheduler overwrites bytes 4-35 of `data` with the real
        // executionIndex at call time, so we encode a zero placeholder.
        bytes memory data = abi.encodeWithSelector(
            this.onScheduledResolve.selector,
            uint256(0),   // placeholder: overwritten by Scheduler
            marketId
        );
